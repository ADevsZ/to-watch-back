...
public interface UserLogsRepository extends JpaRepository<UserLogs, Long> {
    @Query(value = "SELECT * FROM db_to_watch.users_logs WHERE user_id = :userId ORDER BY creation_date DESC", nativeQuery = true)
    List<UserLogs> getAllUserLogsById(@Param("userId") Long userId);
}
